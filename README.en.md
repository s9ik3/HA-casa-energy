...
2. Follow the guided steps:
   ...
   - **Tariff and power**: energy price (currency/kWh), fixed monthly costs, any extra fees, tax rate (always double-check these against your real bill), plus your meter/system's maximum power and alert thresholds — all in a single step
   - **Tariff line items**: a YAML block with your bill's detailed items (see below) — **required**: without these items, the monthly cost would be calculated using only the four fields from the previous step, which usually stay at 0 and would produce a plainly wrong estimate
...
### Tariff line items (required)

This step (part of the initial setup, and reachable at any time from the Options via the "Tariff" step) asks you to paste **all your tariff items at once**, as YAML — a list with one block per item:
...
- **apply_vat**: `false` to exclude that item from the tax rate configured in the previous step; if omitted, tax applies (defaults to `true`)
...
The field is **required**: if you leave it empty, saving is blocked with an explanation — without at least one item, the cost would be calculated using only the four base fields (price/fixed cost/fees/tax), which usually stay at 0 and would produce a plainly wrong estimate with no warning. If you really have a very simple tariff with a single price per kWh and one fixed cost, write two minimal items anyway (`type: per_kwh` and `type: fixed`) instead of leaving the field empty.

If the pasted text isn't valid YAML or is missing a required field, the step flags the specific problem (which item and which field) instead of silently dropping an item. Reopen the step at any time to see and edit the items already saved, pre-filled automatically. Like the simple tariff, items are also frozen on already-closed months when you modify them (see below).
